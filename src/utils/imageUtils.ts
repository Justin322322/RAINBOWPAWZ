...
/**
 * Get profile picture URL with proper cache busting and fallback
 * @param profilePicturePath The profile picture path from database
 * @param userId Optional user ID for fallback
 * @returns The profile picture URL with cache busting
 */
export function getProfilePictureUrl(profilePicturePath: string | null | undefined, userId?: string | number): string {
  ...
  // Always add cache busting for profile pictures to ensure fresh images
  return getImagePath(profilePicturePath, true);
}
