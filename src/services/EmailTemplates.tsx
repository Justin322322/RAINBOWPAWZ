// Consolidated email templates for RainbowPaws
...
<head>
  ...
  <style>
    ...
    .button { display: inline-block; background-color: #10B981; color: white; padding: 12px 24px; text-decoration: none; border-radius: 25px; margin: 20px 0; font-weight: normal; }
    .info-box { background-color: #f9f9f9; border: 1px solid #eee; border-radius: 4px; padding: 15px; margin: 15px 0; }
  </style>
</head>
<body>
  <div class="container">
...
export const createWelcomeEmail = (firstName: string, accountType: 'personal' | 'business') => {
  ...
  const accountSpecificContent = accountType === 'personal'
    ? `
  ...
  const content = `
    ...
    ` : ''}
  `;
  return { subject, html: baseEmailTemplate(content) };
};
...
export const createPasswordResetEmail = (resetToken: string) => {
  ...
  <head>
    ...
    <style>
      body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0; }
      .container { max-width: 600px; margin: 0 auto; padding: 20px; }
      h1 { color: #10B981; text-align: center; margin: 20px 0; font-size: 24px; }
      .content { padding: 0 20px; }
      .button { display: inline-block; background-color: #10B981; color: white !important; padding: 12px 24px; text-decoration: none; border-radius: 25px; margin: 20px 0; font-weight: normal; }
      .footer { margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee; text-align: center; font-size: 14px; color: #666; }
...
export const createBookingStatusUpdateEmail = (bookingDetails: {
  ...
  switch (bookingDetails.status) {
    case 'pending':
      ...
      _additionalInfo = 'Your booking has been submitted and is awaiting confirmation from the service provider.';
      timelineHtml = createTimelineHtml('pending');
      break;
    case 'confirmed':
      statusText = 'confirmed';
      _additionalInfo = 'We have confirmed your booking. Please arrive on time for your appointment.';
      timelineHtml = createTimelineHtml('confirmed');
      break;
    case 'in_progress':
      statusText = 'in progress';
      _additionalInfo = 'Your pet is being cared for with the utmost respect and compassion.';
      timelineHtml = createTimelineHtml('in_progress');
      break;
    case 'completed':
      statusText = 'completed';
      _additionalInfo = 'Your service has been completed. Thank you for choosing our services during this difficult time.';
      timelineHtml = createTimelineHtml('completed');
      break;
    case 'cancelled':
      statusText = 'cancelled';
      _additionalInfo = 'If you have any questions about this cancellation, please contact us.';
      timelineHtml = '';
      break;
  }
...
function createTimelineHtml(currentStatus: Exclude<BookingStatus, 'cancelled'>): string {
  const steps = [
...
export const createBusinessVerificationEmail = (businessDetails: {
  ...
  return { subject, html: baseEmailTemplate(statusContent) };
};
