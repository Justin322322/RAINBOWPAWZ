          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <div className="flex items-center justify-center w-10 h-10 bg-emerald-100 rounded-2xl">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {pkg.supportedPetTypes.map((pet, idx) => (
                  <Badge key={idx} variant="outline" size="sm" className="rounded-2xl">
                    {pet}
                  </Badge>
                ))}
              </div>
                  ...
                  <div key={idx} className="flex items-start gap-3 p-4 bg-gray-50 rounded-2xl">
                    {image && (
                      <Image
                        ...
                        width={40}
                        height={40}
                        className="h-10 w-10 rounded-2xl object-cover border-2 border-gray-200 flex-shrink-0"
                        unoptimized
                      />
                    )}
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6">
            ...
            <div className="space-y-3">
              {pkg.addOns.map((addon: any, idx) => {
                ...
                return (
                  <div key={idx} className="flex items-center justify-between p-4 bg-amber-50 rounded-2xl border border-amber-200">
                    <div className="flex items-center gap-3">
                      {image && (
                        <Image
                          ...
                          width={40}
                          height={40}
                          className="h-10 w-10 rounded-2xl object-cover border-2 border-amber-200 flex-shrink-0"
                          unoptimized
                        />
                      )}
                    ...
                    {price && (
                      <Badge variant="outline" className="text-amber-700 border-amber-300 bg-amber-100 rounded-2xl">
                        +₱{formatPrice(price)}
                      </Badge>
                    )}
                  </div>
